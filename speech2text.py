...
# --- NEW IMPORTS ---
from agent import run_agent   # your Palace Resorts agent
import queue
...
def on_message(ws, message):
    """Handle AssemblyAI real-time transcript messages."""
    try:
        data = json.loads(message)
        msg_type = data.get('type')

        if msg_type == "Begin":
            print(f"\nSession started: {data.get('id')}")
            ...
            if transcript:
                ...
                if formatted:
                    # Send finalized sentence to the AI agent
                    agent_thread = threading.Thread(
                        target=process_with_agent, args=(transcript,))
...
def process_with_agent(transcript_text: str):
    """Send the finalized transcript to your Gemini call-center agent."""
    try:
        print("\n\n--- Sending to AI Agent ---")
        response = run_agent(transcript_text)
        print("\n🤖 Agent Response:")
        import json
